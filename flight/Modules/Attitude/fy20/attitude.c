...
#define GRAV         9.81f
#define ACCEL_SCALE  (GRAV * 0.00244f)
...
//		accel_test = PIOS_ADXL345_Test();
		...
		gyro_queue = xQueueCreate(1, sizeof(float) * 6);
...
static int32_t updateSensors(AccelsData * accels, GyrosData * gyros)
{
	...
	gyros->y = (gyro[3] - GYRO_NEUTRAL) * gyroGain;
	gyros->z = -(gyro[5] - GYRO_NEUTRAL) * gyroGain;
	...
	y = accel_data.y;  //-
	z = -accel_data.z;
	float accel[3] = {(float) x , (float) y , (float) z };
